...
router.post("/reserve/:id", (req, res) => {
  ...
  let programInfo = `SELECT image, id As pro_id, pro_nm, pro_price FROM programmes WHERE id=${proId}`;
  let tourInfo = `SELECT id,company_nm FROM companies WHERE id =${compId}`;
    ...
    conn.query(tourInfo, (err, tourows) => {
      ...
      conn.query(paymentType, (err, paymentrows) => {
        ...
        conn.query(hotelType, (err, hotelrows) => {
          if (err) throw err;
          res.render("reservations/bookingForm", {
            programmes: prorows,
            program: prorows[0],
            tourInfo: tourows,
