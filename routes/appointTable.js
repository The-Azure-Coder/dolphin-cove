...
`;
    ...
    conn.query(tableQuery, (err, rows) => {
      if (err) throw err;
      res.render("reservations/appointTable", {
        appointments: rows,
...
router.post("/update", (req, res) => {
  ...
  conn.query(updatePay, (err, payRows) => {
    if (err) throw err;
    let bookingSql = `update bookings SET company_id='${req.body.comp_id}',  hotel_id='${req.body.hot_id}',  guest_fn='${req.body.guest_fn}', guest_ln='${req.body.guest_ln}' where id =${req.body.book_id}`;
